...
class sim:
    ...
    def delete_from_reservation_station(self, instruction):
    ...
    def remove_instruction_reservation_station(self, instruction):
    ...
    def Execute(self):
        ...
        for ready_instruction in ready_list:
            self.remove_instruction_reservation_station(ready_instruction)
    ...
    def issue(self):
        ...
        ready_list = []
        execution_cap = self.peak_fetch_dispatch_issue_rate + 10000 - len(self.execute_list)

        for issue_instruction in self.issue_list:

            value = self.reservation_station.get(issue_instruction["index"])
            if (execution_cap > 0 and value != None and self.IsReservationStationInstructionReady(
                    issue_instruction) == True):
...
simulator = sim(data, 64, 1)

# In[37]:


simulator.main()
simulator.validate_file("pipe_64_1_perl.txt")
